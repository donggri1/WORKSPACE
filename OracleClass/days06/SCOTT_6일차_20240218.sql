...
SELECT
    TO_CHAR(SYSDATE ,'BC')
    ,TO_CHAR(SYSDATE ,'Q') --1분기1~3 2분기4~6 3분기7~9 4분기 10~ 12
    ,TO_CHAR(SYSDATE ,'MI')50
    ,TO_CHAR(SYSDATE ,'SS')56
...
SELECT ename,hiredate
        ,TO_CHAR(hiredate,'DL')--1981년 12월 3일 목요일
        ,TO_CHAR(SYSDATE ,'TS')--오후 3:51:36
        
        ,TO_CHAR(CURRENT_TIMESTAMP,'HH24:MI:SS.FF9')
FROM emp;

-- 문제 오늘날짜를 TO_CHAR() 함수를 사용하여 출력
--  2024년 02월 20일 오후 16:05:29(화)
--  문자열로 출력.

SELECT 
        TO_CHAR(SYSDATE,'YYYY"년" MM"월" DD"일" PM HH:MI:SS(DY)')
       ,SUBSTR( TO_CHAR(SYSDATE,'DL'),1,13)||TO_CHAR(SYSDATE,'TS(DY)')
FROM dual;
...
SELECT 
      TO_DATE( '0821','MMDD' ) -- [24]/08/21
      ,TO_DATE( '2023','YYYY') -- 23/[02]/[01]
      ,TO_DATE( '202312','YYYYMM') -- 23/12/[01]
      ,TO_DATE('23년01월12일','YY"년"MM"월"DD"일"')
FROM dual;
...
if(a==b){c}else if(a=d){e}else if(a=f){g}else{h}
...
SELECT name,ssn
        ...
        ,REPLACE(REPLACE(MOD(SUBSTR(ssn,8,1),2),0,'여자'),1,'남자')
        ,DECODE(MOD(SUBSTR(ssn,8,1),2),0,'여자','남자')
FROM insa

--[문제] 17-2.  insa 테이블에서 오늘을 기준으로 생일이 지남 여부를 출력하는 쿼리를 작성하세요 . 
 --  ( '지났다', '안지났다', '오늘 ' 처리 )              
 ...
 SELECT DECODE((SIGN(SUBSTR(ssn,3,4)-TRUNC(TO_CHAR(SYSDATE,'MMDD')))),0,'오늘',-1,'지남',1,'안지남')생일
 FROM insa;
 ...
 SELECT 
      SUBSTR(ssn,3,4)
        ,SUBSTR(ssn,3,4)-TRUNC(TO_CHAR(SYSDATE,'MMDD'))ㅇ
        ,SIGN(SUBSTR(ssn,3,4)-TRUNC(TO_CHAR(SYSDATE,'MMDD')))ㄴ
        ,REPLACE(REPLACE(REPLACE(SIGN(SUBSTR(ssn,3,4)-TRUNC(TO_CHAR(SYSDATE,'MMDD'))),1,'안지남'),-1,'지남'),0,'오늘')ㄹ
 FROM insa;
...
SELECT empno,ename,sal,deptno
        ...
        ,sal+sal*DECODE(deptno,10,0.15,20,0.1,30,0.5)인상된금액
        ,(sal+NVL(comm,0))*(DECODE(deptno,10,0.15,20,0.1,30,0.5)) 인상금액
FROM emp;
-- 조건 20번 부서원들은 급여의 15%인상해서 급여 
-- 조건 30번 부서원들은 급여의 15%인상해서 급여 
