...
SELECT *
FROM departments;
-- 5)JOBS 잡 테이블 (잡ID, 잡이름, 최소SAL, 최대 SAL)
SELECT *
FROM jobs;
-- 6)EMPLOYEES 사원테이블(사원ID, 이름, 성, 이메일, 폰번호 , 입사일자, 잡ID, SAL,등등)
SELECT *
FROM employees;

-- 7)JOB_HISTORY (사원ID,시작일,종료일,잡ID,부서ID)
DESC job_history;
SELECT *
FROM job_history;
...
SELECT *
FROM employees;
--위의 테이블에서 사원번호, 사원이름만 출력
-- *** first_name,last_name = name 컴럼으로 출력
